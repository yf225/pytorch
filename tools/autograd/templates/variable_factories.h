...
#undef TENSOR
  ...
  ArrayRefWrapper(std::initializer_list<ScalarWrapper> values) {
    at::AutoNonVariableTypeMode non_var_type_mode(true);
    std::vector<int64_t> tensor_sizes = {values.size()};
    tensor_ = at::empty(tensor_sizes, at::TensorOptions().dtype(values.begin()->scalar_type_).device(at::kCPU).is_variable(false));
    size_t index = 0;
    for (const auto& elem : values) {
      at::NoGradGuard guard;
      tensor_[index].fill_(elem.scalar_);
      index++;
    }
  }
...
inline at::Tensor tensor(                                                
      std::initializer_list<ScalarWrapper> values, const at::TensorOptions& options) { 
    return torch::tensor(ArrayRefWrapper(values), options);                
  }                                                                        

inline at::Tensor tensor(std::initializer_list<ScalarWrapper> values) {              
    return torch::tensor(ArrayRefWrapper(values));                         
  }                                                                        

#define TENSOR(T, S)                                                       \
  inline at::Tensor tensor(T value, const at::TensorOptions& options) {    \
    return torch::tensor(ArrayRefWrapper(at::ArrayRef<T>(value)), options);                 \
  }                                                                        \
  inline at::Tensor tensor(T value) {                                      \
    return torch::tensor(ArrayRefWrapper(at::ArrayRef<T>(value)));                          \
  }
AT_FORALL_SCALAR_TYPES_AND3(Bool, Half, BFloat16, TENSOR)
#undef TENSOR
...
#define TENSOR(T, S) \
/* base case */ \
inline std::vector<int64_t> _compute_tensor_sizes(std::initializer_list<T> init_list) { \
  return {init_list.size()}; \
}
AT_FORALL_SCALAR_TYPES_AND3(Bool, Half, BFloat16, TENSOR)
#undef TENSOR

template <typename init_list_t>
inline std::vector<int64_t> _compute_tensor_sizes(std::initializer_list<init_list_t> init_list) {
  std::vector<int64_t> tensor_sizes = _compute_tensor_sizes(*(init_list.begin()));
  tensor_sizes.insert(tensor_sizes.begin(), init_list.size());
  return tensor_sizes;
}

#define TENSOR(T, S) \
/* base case */ \
inline void _fill_tensor(at::Tensor tensor, std::initializer_list<T> init_list) { \
  size_t index = 0; \
  for (const auto& elem : init_list) { \
    at::NoGradGuard guard; \
    tensor[index].fill_(elem); \
    index++; \
  } \
}
AT_FORALL_SCALAR_TYPES_AND3(Bool, Half, BFloat16, TENSOR)
...
#define TENSOR(T, S) \
inline at::Tensor tensor(std::initializer_list<std::initializer_list<std::initializer_list<T>>> init_list, const at::TensorOptions& options = {}) { \
  std::vector<int64_t> tensor_sizes = _compute_tensor_sizes(init_list); \
  at::Tensor tensor = ([&]() { \
    at::AutoNonVariableTypeMode non_var_type_mode(true); \
    return at::empty(tensor_sizes, at::TensorOptions(options).dtype(at::k##S).device(at::kCPU).is_variable(false)); \
...
#define TENSOR(T, S) \
inline at::Tensor tensor(std::initializer_list<std::initializer_list<std::initializer_list<std::initializer_list<T>>>> init_list, const at::TensorOptions& options = {}) { \
  std::vector<int64_t> tensor_sizes = _compute_tensor_sizes(init_list); \
  at::Tensor tensor = ([&]() { \
    at::AutoNonVariableTypeMode non_var_type_mode(true); \
    return at::empty(tensor_sizes, at::TensorOptions(options).dtype(at::k##S).device(at::kCPU).is_variable(false)); \
...
#define TENSOR(T, S) \
inline at::Tensor tensor(std::initializer_list<std::initializer_list<std::initializer_list<std::initializer_list<std::initializer_list<T>>>>> init_list, const at::TensorOptions& options = {}) { \
  std::vector<int64_t> tensor_sizes = _compute_tensor_sizes(init_list); \
  at::Tensor tensor = ([&]() { \
    at::AutoNonVariableTypeMode non_var_type_mode(true); \
    return at::empty(tensor_sizes, at::TensorOptions(options).dtype(at::k##S).device(at::kCPU).is_variable(false)); \
...
#define TENSOR(T, S) \
inline at::Tensor tensor(std::initializer_list<std::initializer_list<std::initializer_list<std::initializer_list<std::initializer_list<std::initializer_list<T>>>>>> init_list, const at::TensorOptions& options = {}) { \
  std::vector<int64_t> tensor_sizes = _compute_tensor_sizes(init_list); \
  at::Tensor tensor = ([&]() { \
    at::AutoNonVariableTypeMode non_var_type_mode(true); \
    return at::empty(tensor_sizes, at::TensorOptions(options).dtype(at::k##S).device(at::kCPU).is_variable(false)); \
