...
#undef TENSOR
    ListInitTensor(std::initializer_list<ListInitTensor> init_list) :
      ...
      // NOTE: `at::tensor({})` doesn't work at the moment because we would need to solve the
      // ambiguous overload problem (see https://github.com/pytorch/pytorch/pull/26210#discussion_r325336686).
      // If the user wants to create an empty tensor, they can use `at::randn({0})` for now.
      ...
      for (const auto& elem : init_list) {
        // NOTE: Currently `at::tensor(...)` doesn't support mixed data types
        // (i.e. `at::tensor({{bool, 2.0}})` doesn't work). We might be able to
