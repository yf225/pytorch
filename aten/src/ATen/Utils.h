...
struct ComputeRequiresGrad : IterArgs<ComputeRequiresGrad> {
  ...
  void operator()(const at::Tensor& tensor) {
    if (tensor.defined() && tensor.is_variable() && tensor.requires_grad()) {
      out = true;
    }
  }
  bool short_circuit() {
...
inline bool compute_requires_grad(Args&&... args) {
  // yf225 TODO: move GradMode to ATen! Or should we find a way to merge those two thread-local guards?
