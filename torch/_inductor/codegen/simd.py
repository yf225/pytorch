...
class SIMDScheduling(BaseScheduling):
    ...
    def _codegen_single_template(
        ...
        *,
        ...
        standalone_epilogues = kernel.get_standalone_epilogues(epilogue_nodes)
        ...
        with kernel:
            ...
            # get_epilogues_for_subgraph() returns the appropriate epilogue nodes
            # per output.  Standard templates: all epilogues per body.
            # External templates: per-output assignment.
            store_output_count = sum(
                1 for k in kernel.subgraph_bodies if k.startswith("<STORE_OUTPUT_")
            )
            for i in range(store_output_count):
                subgraph_epilogues = kernel.get_epilogues_for_subgraph(
                    i, epilogue_nodes
                )
                with kernel.set_subgraph_body(
                    kernel._get_store_output_subgraph_name(i)
                ):
                    kernel._apply_template_epilogue(i)
                    kernel.codegen_body()
