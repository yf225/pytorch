...
# =============================================================================
# PALLAS VIEW TRACKING SYSTEM
# =============================================================================
#
# The Pallas backend needs to know how tensor views were constructed to generate
# correct reshape/transpose operations. This is tracked via "view chains".
#
# KEY CONCEPTS:
#
# 1. View ID: A unique integer identifying a specific view operation chain.
#    Generated by PallasViewTracker.next_view_id().
#
# 2. Transform Chain: A list of dicts describing view operations from buffer to current view.
#    Example: [{"op": "permute", "dims": [2,0,1]}, {"op": "expand", "expand_dims": [0]}]
#
# 3. PallasStride Marker: A sympy.Function embedded in index expressions that carries
#    (stride_value, dim_index, view_id, iter_var_pos). Enables Pallas codegen to
#    extract access patterns without guessing from strides.
#
# LIFECYCLE:
#
#   ┌─────────────────┐
#   │ View Operation  │  (expand, permute, reshape, unsqueeze, slice, unfold)
#   │ in lowering.py  │
#   └────────┬────────┘
#            │ calls PallasViewTracker.register(x.data, {"op": ..., ...})
#            ▼
#   ┌─────────────────┐
#   │ Transform Entry │  stored in V.graph._pallas_view_entries[view_id]
#   │ Registry        │  each entry has transforms + source_view_id link
#   └────────┬────────┘
#            │ view_id stored on View/ExpandView node or layout
#            ▼
#   ┌─────────────────┐
#   │ make_loader()   │  outer view sets context via PallasViewTracker.context(view_id)
#   │ make_indexer()  │  inner FixedLayout reads context, embeds in PallasStride markers
#   └────────┬────────┘
#            │
#            ▼
#   ┌─────────────────┐
#   │ PallasKernel    │  extracts view_id from PallasStride in index expressions
#   │ codegen         │  looks up chain → generates reshape/transpose code
#   └─────────────────┘
#
# THREAD-LOCAL CONTEXT:
#
# When make_loader() is called, outer views need to tell inner layouts which view_id
# to embed in PallasStride markers. This is done via thread-local context:
#
#     with PallasViewTracker.context(self.pallas_view_id):
#         return inner(reindex(idx))  # FixedLayout.make_indexer() reads context
#
# This pattern is necessary because:
# - make_loader() creates a closure that's called later during codegen
# - We can't pass view_id through the closure signature (would break interface)
# - Multiple nested views may be active simultaneously
#
# =============================================================================
...
# Other backends ignore this symbol.
...
class PallasStride(sympy.Function):
    ...
    PallasStride(stride_value, dim_index, view_id, iter_var_pos) preserves stride metadata
    ...
    Args:
        ...
        dim_index: The dimension index in the current layout
        view_id: Optional unique ID linking to the transform entry in V.graph._pallas_view_entries
        iter_var_pos: Position of iteration variable in the buffer layout (0, 1, 2, ...).
                      This enables unambiguous mapping from buffer position to dimension,
                      even when multiple dimensions have the same stride (e.g., size-1 dims).

    This enables Pallas to detect permutations:
    - Index `i0 * PallasStride(1, 0, 42, 0) + i1 * PallasStride(8, 1, 42, 1)` indicates strides (1, 8)
    - view_id 42 maps to the transform chain that produced this layout
    - iter_var_pos 0, 1 indicates buffer positions directly

    For sympy interpretation, PallasStride evaluates to its stride value.
    """

    # Tell sympy interpreter to use 'pallas_stride' handler
    _torch_handler_name = "pallas_stride"

    @classmethod
    def eval(cls, stride, dim, view_id=None, iter_var_pos=None):
        # Don't auto-simplify - preserve the marker for Pallas to extract
        # Return None to keep PallasStride(stride, dim, view_id, iter_var_pos) in the expression
        if stride == 0:
            return Integer(0)
        return None
    ...
    def _eval_is_positive(self):
        # Strides can be negative for flipped dimensions
        return None
    ...
    @property
    def view_id(self):
        """Get the view ID linking to transform chain (if present)."""
        return self.args[2] if len(self.args) > 2 else None

    @property
    def iter_var_pos(self):
        """Get the iteration variable position (buffer position) if present."""
        return self.args[3] if len(self.args) > 3 else None


class PallasViewIdMarker(sympy.Function):
    """
    Zero-valued marker that carries view_id information for Pallas codegen.

    This is used when a tensor has all dimensions of size 1 (like shape [1]),
    where no PallasStride markers would normally be created. The marker:
    - Evaluates to 0 numerically (doesn't affect index calculation)
    - Doesn't simplify away (preserves the view_id in the expression)
    - Can be detected via index.atoms(PallasViewIdMarker)

    Usage: result = offset + PallasViewIdMarker(view_id)
    """

    # Tell sympy interpreter to use 'pallas_view_id_marker' handler
    _torch_handler_name = "pallas_view_id_marker"

    @classmethod
    def eval(cls, view_id):
        # Never simplify - always preserve the marker
        return None
...
import threading
_pallas_view_id_context = threading.local()


class PallasViewTracker:
    """
    Centralized tracking for Pallas view transformations.

    This class provides a unified interface for all Pallas view tracking operations.
    It organizes the functionality into logical groups:

    - Backend Detection: is_active()
    - View ID Management: next_view_id()
    - Entry-Based Storage: store_entry(), get_entry(), get_effective_chain()
    - Node Registry: set_node_view_id(), get_node_view_id()
    - View Registration: register()
    - Context Management: get_outer_view_id(), set_outer_view_id(), context()
    - Loader/Indexer Helpers: make_aware_loader(), make_aware_indexer()
    - View Chain Extraction: find_inner_view_id(), collect_chain()
    - Value Helpers: safe_int()

    Registry Storage (on V.graph):
    - _pallas_view_id_counter: int - monotonic counter for unique IDs
    - _pallas_view_entries: dict[int, dict] - view_id -> entry with transforms + source_view_id
    - _pallas_node_view_ids: dict[int, int] - node id() -> view_id (for frozen dataclasses)

    Entry format (stored via store_entry):
    {
        "transforms": [transform_dict, ...],  # Just THIS view's transforms
        "source_view_id": int | None,         # Link to parent view
        "output_shape": list[int] | None,     # Shape after this transform
    }

    Chain reconstruction: get_effective_chain(view_id, buf_shape) follows source_view_id
    links to rebuild the full chain, stopping at materialization boundaries.

    Usage:
        # Check if Pallas is active
        if PallasViewTracker.is_active():
            view_id = PallasViewTracker.register(x.data, {"op": "unsqueeze", "dim": 0})

        # Use context for make_loader
    ...
    # === View ID Management ===
    @staticmethod
    def next_view_id() -> int:
        """Generate a unique view ID for Pallas transform chain tracking."""
        if not hasattr(V.graph, "_pallas_view_id_counter"):
            V.graph._pallas_view_id_counter = 0
        V.graph._pallas_view_id_counter += 1
        return V.graph._pallas_view_id_counter

    # === Entry-Based Storage ===
    @staticmethod
    def store_entry(view_id: int, entry: dict) -> None:
        """Store a view entry with single transform + source link.

        Entry format:
        {
            "transforms": [transform_dict, ...],  # Just THIS view's transforms
            "source_view_id": int | None,         # Parent view_id
            "output_shape": list[int] | None,     # Shape after this transform
        }
        """
        if not hasattr(V.graph, "_pallas_view_entries"):
            V.graph._pallas_view_entries = {}
        V.graph._pallas_view_entries[view_id] = entry

    @staticmethod
    def get_entry(view_id: int) -> Optional[dict]:
        """Retrieve a view entry by view_id."""
        registry = getattr(V.graph, "_pallas_view_entries", {})
        return registry.get(view_id)

    @staticmethod
    def get_effective_chain(view_id: int, actual_buf_shape: Optional[list]) -> list:
        """Reconstruct chain from registry entries. USE AT CODEGEN TIME.

        Use this when you have a view_id (from PallasStride markers) and need
        the transform chain. Supports materialization boundary detection.

        For IR traversal (when you have an IRNode), use collect_chain() instead.

        Key insight: If actual_buf_shape matches the output_shape of a parent entry,
        that parent was materialized and we should not include its transforms.

        Args:
            view_id: The view_id from the index expression
            actual_buf_shape: The actual shape of the buffer being loaded, or None
                             to skip materialization detection and return full chain

        Returns:
            List of transforms to apply (innermost to outermost)
        """
        # Normalize actual_buf_shape to list of ints (if provided)
        if actual_buf_shape is not None:
            actual_shape = [int(s) if hasattr(s, '__int__') else s for s in actual_buf_shape]
        else:
            actual_shape = None  # Skip materialization detection
        ...
        for _ in range(100):  # Safety limit
            ...
            entry = PallasViewTracker.get_entry(current_id)
            if entry is None:
                # Entry not found - stop here
                break

            # Check if we should stop here (parent was materialized)
            # Only do this if we have actual_shape for comparison
            parent_id = entry.get("source_view_id")
            if actual_shape is not None and parent_id is not None:
                parent_entry = PallasViewTracker.get_entry(parent_id)
                if parent_entry is not None:
                    parent_output = parent_entry.get("output_shape")
                    if parent_output is not None:
                        parent_shape = [int(s) if hasattr(s, '__int__') else s for s in parent_output]
                        if actual_shape == parent_shape:
                            # Parent's output matches actual buffer - parent was materialized!
                            # Only include current entry's transforms
                            chain.extend(entry.get("transforms", []))
                            break

            # Include this entry's transforms
            chain.extend(entry.get("transforms", []))
            current_id = parent_id

        # Return innermost-to-outermost order
        return list(reversed(chain))

    # === Node Registry ===
    @staticmethod
    def set_node_view_id(node_id: int, view_id: int) -> None:
        """Store a view_id for an IR node by its id().

        This is needed because IR nodes are frozen dataclasses and can't have attributes added.
        """
        if not hasattr(V.graph, "_pallas_node_view_ids"):
            V.graph._pallas_node_view_ids = {}
        V.graph._pallas_node_view_ids[node_id] = view_id

    @staticmethod
    def get_node_view_id(node_id: int) -> Optional[int]:
        """Get the view_id for an IR node by its id()."""
        registry = getattr(V.graph, "_pallas_node_view_ids", {})
        return registry.get(node_id)

    # === View Registration ===
    @staticmethod
    def _get_node_shape(x: "IRNode") -> Optional[list]:
        """Get the shape of an IRNode.

        Handles StorageBox/TensorBox unwrapping and various node types.
        """
        current = x
        for _ in range(100):  # Safety limit
            if current is None:
            ...
            if node_type in ("StorageBox", "TensorBox"):
                current = getattr(current, "data", None)
                continue
            if hasattr(current, "get_size"):
                try:
                    size = current.get_size()
                    # Convert to list of ints where possible
                    return [int(s) if hasattr(s, '__int__') else s for s in size]
                except Exception:
                    return None
            return None
        return None
    ...
    @staticmethod
    def _compute_output_shape(source_shape: list, transform: dict) -> Optional[list]:
        """Compute the output shape after applying a transform.

        Args:
            source_shape: Shape before transform
            transform: Transform dict with 'op' key

        Returns:
            Output shape, or None if cannot be determined
        """
        ...
        if op == "expand":
            # Expand specifies target_shape directly
            return transform.get("target_shape")

        elif op == "reshape":
            # Reshape specifies to_shape directly
            return transform.get("to_shape")

        elif op == "permute":
            # Permute reorders dimensions according to dims
            dims = transform.get("dims", [])
            if len(dims) == len(source_shape):
                return [source_shape[d] for d in dims]
            return None

        elif op == "unsqueeze":
            # Unsqueeze adds a dimension of size 1
            dim = transform.get("dim", 0)
            result = list(source_shape)
            result.insert(dim, 1)
            return result

        elif op == "squeeze":
            # Squeeze removes a dimension of size 1
            dim = transform.get("dim")
            if dim is not None and 0 <= dim < len(source_shape):
                result = list(source_shape)
            ...
            # Slice reduces one dimension
            dim = transform.get("dim", 0)
            start = transform.get("start", 0)
            end = transform.get("end")
            step = transform.get("step", 1)
            if end is not None and 0 <= dim < len(source_shape):
                result = list(source_shape)
                # Compute sliced dimension size
                slice_size = (end - start + step - 1) // step
                result[dim] = slice_size
                return result
            return None

        elif op == "unfold":
            # Unfold adds a dimension
            size = transform.get("size")
            if size is not None:
                result = list(source_shape) + [size]
                return result
            return None

        # Unknown op - can't determine output shape
        return None

    @staticmethod
    def register(x: "IRNode", transforms: Union[dict, list]) -> Optional[int]:
        """Register Pallas view transformation(s) and return view_id.

        Uses entry-based design: store transform(s) + source link.
        The chain is reconstructed dynamically at codegen time.

        Args:
            x: The source IRNode (input to the view operation)
            transforms: Single transform dict OR list of transform dicts, e.g.:
                {"op": "expand", "expand_dims": [...], "target_shape": [...]}
                {"op": "permute", "dims": [...]}
                {"op": "squeeze", "dim": ...}
                {"op": "slice", "dim": ..., "start": ..., "end": ..., "step": ...}
                {"op": "reshape", "from_shape": [...], "to_shape": [...]}
                Or a list of the above for compound operations (e.g., multiple squeezes)

        Returns:
            view_id if Pallas is active, None otherwise
        """
        ...
        # Find source node's view_id (if it has one)
        source_view_id = PallasViewTracker.find_inner_view_id(x)

        # Compute output shape by applying each transform in sequence
        current_shape = PallasViewTracker._get_node_shape(x)
        for t in transforms:
            if current_shape is not None:
                current_shape = PallasViewTracker._compute_output_shape(current_shape, t)

        view_id = PallasViewTracker.next_view_id()

        # Store in entry-based format
    ...
    # === Context Management ===
    @staticmethod
    def get_outer_view_id() -> Optional[int]:
        """Get the current outer view_id from context (if any)."""
        return getattr(_pallas_view_id_context, 'view_id', None)

    @staticmethod
    def set_outer_view_id(view_id: Optional[int]) -> None:
        """Set the outer view_id context for make_loader() calls."""
        _pallas_view_id_context.view_id = view_id

    @staticmethod
    @contextlib.contextmanager
    def context(view_id: Optional[int]):
        """Context manager for temporarily setting Pallas view_id.

        Consolidates the repeated save/restore pattern used in lowering.py.
        Always saves and restores the view_id, even when setting to None.

        Usage:
            with PallasViewTracker.context(view_id):
                result = inner_loader(idx)
        """
        old_view_id = PallasViewTracker.get_outer_view_id()
        PallasViewTracker.set_outer_view_id(view_id)
        try:
            yield
        finally:
    ...
    def make_aware_loader(
        inner: Callable[[Sequence[Expr]], "OpsValue"],
        reindex: Callable[[Sequence[Expr]], Sequence[Expr]],
        view_id: Optional[int]
    ) -> Callable[[Sequence[Expr]], "OpsValue"]:
        """Create a loader that propagates Pallas view_id context.

        Consolidates the repeated pattern in ExpandView, PermuteView, View.
        When view_id is set, it's propagated via thread-local context so that
        inner layouts (FixedLayout) embed the correct view_id in PallasStride markers.

        Args:
            inner: The inner data's make_loader() result
            reindex: The view's reindexing function
            view_id: Optional Pallas view_id to propagate

        Returns:
            A loader function that handles Pallas context propagation
        """
        if view_id is None:
        ...
        def loader(idx: Sequence[Expr]) -> "OpsValue":
            # Only set context if not already set by an outer view
            # The outermost view_id should win because it represents the complete chain
            # from buffer to final view. Inner views should not overwrite.
            outer_view_id = PallasViewTracker.get_outer_view_id()
            should_set = outer_view_id is None

            if should_set:
                PallasViewTracker.set_outer_view_id(view_id)
            try:
                result = inner(reindex(idx))
            finally:
                if should_set:
                    PallasViewTracker.set_outer_view_id(None)
            return result
        return loader

    @staticmethod
    def make_aware_indexer(
        inner: Callable[[Sequence[Expr]], Expr],
        reindex: Callable[[Sequence[Expr]], Sequence[Expr]],
        view_id: Optional[int],
        expand_callback: Optional[Callable[[Sequence[Expr], Expr], Expr]] = None
    ) -> Callable[[Sequence[Expr]], Expr]:
        """Create an indexer that propagates Pallas view_id via context.

        Uses the same context mechanism as make_aware_loader() for consistency.
        The outer view_id is set in thread-local context, which _fixed_indexer()
        reads to embed the correct view_id in PallasStride markers.

        Args:
            inner: The inner data's make_indexer() result
            reindex: The view's reindexing function
            view_id: Optional Pallas view_id to propagate
            expand_callback: Optional callback to add expand dimension markers (ExpandView only)

        Returns:
            An indexer function that handles Pallas view_id propagation
        """
        if view_id is None:
            def indexer(idx: Sequence[Expr]) -> Expr:
                result = inner(reindex(idx))
                if expand_callback is not None:
                    result = expand_callback(idx, result)
                return result
            return indexer

        def indexer(idx: Sequence[Expr]) -> Expr:
            # Use context mechanism (same as make_aware_loader)
            # Only set context if not already set by an outer view
            outer_view_id = PallasViewTracker.get_outer_view_id()
            should_set = outer_view_id is None
            ...
            try:
                result = inner(reindex(idx))
                # Apply expand callback within context scope
                if expand_callback is not None:
                    result = expand_callback(idx, result)
            finally:
                if should_set:
    ...
    # === View Chain Extraction ===
    @staticmethod
    def find_inner_view_id(node: "IRNode") -> Optional[int]:
        """Find the innermost view_id from a data chain.

        Checks:
        - View nodes (with pallas_view_id field)
        - ReinterpretView nodes (with pallas_view_id in layout)
        - ExpandView nodes (with pallas_view_id field)
        - Pointwise nodes (extract view_id from PallasStride in inner_fn)
        - StorageBox/TensorBox (unwrap and check data)
        """
        current = node
        for _ in range(100):  # Safety limit
            if current is None:
            ...
            # Unwrap StorageBox and TensorBox
            if node_type in ("StorageBox", "TensorBox"):
                current = getattr(current, "data", None)
                continue

            # Check for View/SliceView with pallas_view_id
            # SliceView extends View but has a different type name
            if node_type in ("View", "SliceView"):
                view_id = getattr(current, "pallas_view_id", None)
                if view_id is not None:
                    return view_id

            # Check for ExpandView with pallas_view_id
            if node_type == "ExpandView":
                view_id = getattr(current, "pallas_view_id", None)
                if view_id is not None:
                    return view_id

            # Check for ReinterpretView with layout
            if node_type == "ReinterpretView":
                layout = getattr(current, "layout", None)
                if layout is not None:
                    view_id = getattr(layout, "pallas_view_id", None)
                    if view_id is not None:
                        return view_id

            # Check for Pointwise - first check registry (set by wrap_index_loader),
            # then fallback to extracting from PallasStride in inner_fn
            if node_type == "Pointwise":
                # First check the node registry (set by wrap_index_loader for index tensors)
                # This is critical for finding index_source view_ids when index tensors
                # go through ExpandView for broadcasting
                view_id = PallasViewTracker.get_node_view_id(id(current))
                if view_id is not None:
                    return view_id
                # Fallback: extract view_id from PallasStride in inner_fn
                view_id = PallasViewTracker.extract_view_id_from_pointwise(current)
                if view_id is not None:
                    return view_id
    ...
    @staticmethod
    def collect_chain(node: "IRNode") -> list:
        """Traverse IR tree to collect view transforms. USE AT LOWERING TIME.

        Use this when you have an IRNode and need to discover its view chain.
        Handles both registered and unregistered view nodes.

        For codegen (when you have a view_id), use get_effective_chain() instead.

        Returns a list of transform dicts from innermost (closest to buffer) to outermost.

        Handles various node types:
        - View/SliceView/ExpandView with pallas_view_id: retrieve stored chain
        - ReinterpretView with pallas_view_id in layout: retrieve stored chain
        - Pointwise: extract view_id from PallasStride in inner_fn
        - PermuteView: extract dims directly from node
        - ReinterpretView with pallas_origin: use the origin dict
        """
        chain = []
        current = node
        ...
        for _ in range(100):  # Safety limit
            ...
            # Check for View/SliceView with pallas_view_id - use stored chain directly
            # SliceView extends View but has a different type name
            if node_type in ("View", "SliceView"):
                view_id = getattr(current, "pallas_view_id", None)
                if view_id is not None:
                    # Reconstruct chain from entries (includes all prior transforms)
                    stored_chain = PallasViewTracker.get_effective_chain(view_id, None)
                    assert stored_chain, f"view_id {view_id} registered on {node_type} but chain not found"
                    # This chain already contains all transforms from buffer to here
                    return stored_chain
                # No view_id, continue traversing
                current = getattr(current, "data", None)

            # Check for ExpandView with pallas_view_id - use stored chain directly
            elif node_type == "ExpandView":
                view_id = getattr(current, "pallas_view_id", None)
                if view_id is not None:
                    # Reconstruct chain from entries (includes all prior transforms)
                    stored_chain = PallasViewTracker.get_effective_chain(view_id, None)
                    assert stored_chain, f"view_id {view_id} registered on ExpandView but chain not found"
                    # This chain already contains all transforms from buffer to here
                    return stored_chain
                # No view_id, continue traversing
                current = getattr(current, "data", None)

            # Check for Pointwise - check node view_id registry first, then extract from PallasStride
            elif node_type == "Pointwise":
                # First check if view_id was stored in registry (by wrap_index_loader)
                # since frozen dataclasses don't allow attribute assignment
                view_id = PallasViewTracker.get_node_view_id(id(current))
                if view_id is not None:
                    stored_chain = PallasViewTracker.get_effective_chain(view_id, None)
                    assert stored_chain, f"view_id {view_id} from node registry but chain not found"
                    return stored_chain + list(reversed(chain))
                # Fallback: try to extract from PallasStride in inner_fn
                view_id = PallasViewTracker.extract_view_id_from_pointwise(current)
                if view_id is not None:
                    stored_chain = PallasViewTracker.get_effective_chain(view_id, None)
                    assert stored_chain, f"view_id {view_id} extracted from Pointwise but chain not found"
                    return stored_chain + list(reversed(chain))
                # Pointwise doesn't have .data, break
                break

            # Check for PermuteView - explicit permutation
            elif node_type == "PermuteView":
                chain.append({
                    "op": "permute",
                    "dims": [int(d) for d in current.dims],
                })
                current = getattr(current, "data", None)

            # Check for ReinterpretView
            elif node_type == "ReinterpretView":
                # First check for view_id in layout
                layout = getattr(current, "layout", None)
                if layout is not None:
                    view_id = getattr(layout, "pallas_view_id", None)
                    if view_id is not None:
                        ...
                        stored_chain = PallasViewTracker.get_effective_chain(view_id, None)
                        assert stored_chain, f"view_id {view_id} in layout but chain not found"
                        # Prepend stored chain to what we've collected so far
                        return stored_chain + list(reversed(chain))

                # Fallback: check for pallas_origin
                pallas_origin = getattr(current, "pallas_origin", None)
                if pallas_origin is not None:
                    chain.append(pallas_origin)
                current = getattr(current, "data", None)

            # Other view types - just traverse
            elif hasattr(current, "data"):
                current = current.data
            else:
                break

        # Return innermost-first order (reverse of traversal order)
        return list(reversed(chain))

    @staticmethod
    def extract_view_id_from_pointwise(pointwise_node: "IRNode") -> Optional[int]:
        """Extract view_id from PallasStride markers in a Pointwise node's inner_fn.

        This is robust because it extracts the explicitly-set view_id from PallasStride,
        not relying on fragile size matching or stride sorting.
        """
        from unittest.mock import patch

        inner_fn = getattr(pointwise_node, "inner_fn", None)
        if inner_fn is None:
            return None

        ranges = getattr(pointwise_node, "ranges", [])
        if not ranges:
            return None
        ...
        try:
            ...
            # Create dummy index symbols
            dummy_idx = [symbols(f"_pallas_dummy_{i}") for i in range(len(ranges))]

            # Handler to capture view_id from PallasStride in load expressions
            class ViewIdExtractor:
                def __init__(self):
                    self.view_id = None

                def load(self, name, index):
                    if hasattr(index, 'atoms'):
                        for ps in index.atoms(PallasStride):
                            vid = ps.view_id
                            if vid is not None and int(vid) >= 0:
                                self.view_id = int(vid)
                                break
                    return index

                def __getattr__(self, name):
                    # Return a no-op for other ops
                    return lambda *args, **kwargs: args[0] if args else 0

            extractor = ViewIdExtractor()
            # Allow indexing during extraction to avoid FlexibleLayout assertion
            with V.set_ops_handler(extractor), patch.object(
                FlexibleLayout, "allow_indexing", True
            ):
                inner_fn(dummy_idx)

            return extractor.view_id
        except Exception:
    ...
    @staticmethod
    def safe_int(val):
        """Safely convert value to int for Pallas view chains.

        For symbolic shapes (sympy symbols), returns the symbol as-is.
        For concrete values, returns int.
        This allows view chains to work with dynamic shapes.
        """
        # For sympy expressions, check if it's a concrete number
        if isinstance(val, sympy.Basic):
            if val.is_number:
                return int(val)
            else:
                # Symbolic - keep as sympy expression
                return val
        # For regular Python values, try to convert
        try:
            return int(val)
...
def try_match_insignificant_strides(
    ...
    new_layout = FixedLayout(
        ...
        getattr(old_layout, 'pallas_view_id', None),
    )
...
class Reduction(Loops):
    ...
    # Original dimension indices being reduced (for Pallas codegen)
    reduction_dim_indices: Optional[tuple[int, ...]] = None
    ...
    def create(
        ...
        # Check if Pallas backend is active - skip unrolling for Pallas
        # because Pallas uses 2D canonical form that requires proper reduction dims
        is_pallas = PallasViewTracker.is_active()
        ...
        if (
            ...
            and not is_pallas  # Don't unroll for Pallas backend
        ):
        ...
        out = TensorBox.create(
            Reduction(
                ...
                reduction_dim_indices=reduction_dim_indices,
            )
        )
        return out
...
def _fixed_indexer(
    ...
    pallas_view_id: Optional[int] = None,
    ...
    Args:
        ...
        stride: Strides for each dimension
        offset: Base offset in the buffer
        pallas_view_id: Optional view ID for Pallas to link to transform chain
        pallas_orig_dims: Optional mapping from current position to original dimension.
                         For permute(1, 0), this would be [1, 0] meaning position 0 came
                         from original dim 1. This enables unambiguous dimension tracking
                         even when multiple dims have the same stride.
    """
    # Check if Pallas backend is active
    is_pallas = PallasViewTracker.is_active()
    ...
    def indexer(index: Sequence[int]) -> int:
        ...
        # Check for outer view_id from context (set by ExpandView/View/embedding/gather make_loader())
        # This allows the correct view_id to be used for each view chain
        outer_view_id = PallasViewTracker.get_outer_view_id()
        effective_view_id = outer_view_id if outer_view_id is not None else pallas_view_id

        # For Pallas: Track if we added any PallasStride markers
        # If all dimensions have size 1, we need to add a marker to carry the view_id
        added_pallas_marker = False

        for dim_idx, (idx, st, sz) in enumerate(zip(index, stride, size)):
            if sz != 1:
                added_pallas_marker = True
                if st == 0 and is_pallas:
                    # Mark expand dimension for Pallas codegen with position tracking
                    # Use PallasStride with PALLAS_EXPAND_STRIDE to preserve iter_var_pos
                    orig_dim = pallas_orig_dims[dim_idx] if pallas_orig_dims else dim_idx
                    view_id = Integer(effective_view_id) if effective_view_id is not None else Integer(-1)
                    result = result + idx * PallasStride(PALLAS_EXPAND_STRIDE, Integer(orig_dim), view_id, Integer(dim_idx))
                elif is_pallas:
                    # Mark stride with PallasStride for Pallas to extract access pattern
                    # dim_index: the ORIGINAL dimension this position came from
                    # iter_var_pos: the current buffer position (dim_idx)
                    # For permute(1, 0): position 0 has orig_dim=1, position 1 has orig_dim=0
                    orig_dim = pallas_orig_dims[dim_idx] if pallas_orig_dims else dim_idx
                    view_id = Integer(effective_view_id) if effective_view_id is not None else Integer(-1)
                    # Keep st as-is if it's already a sympy expression (e.g., symbolic stride)
                    st_expr = st if isinstance(st, sympy.Basic) else Integer(st)
                    result = result + idx * PallasStride(st_expr, Integer(orig_dim), view_id, Integer(dim_idx))
                else:
                    result = result + idx * st

        # For Pallas: If all dimensions have size 1 but we have a view_id, add a marker
        # This preserves the view_id information in the index expression for tensors like shape [1]
        # PallasViewIdMarker evaluates to 0 but doesn't simplify away
        if is_pallas and not added_pallas_marker and effective_view_id is not None:
            view_id = Integer(effective_view_id)
            # Add a zero-valued marker that carries the view_id
            result = result + PallasViewIdMarker(view_id)
...
class BaseView(IRNode):
    ...
    def _make_pallas_aware_loader(self) -> Callable[[Sequence[Expr]], OpsValue]:
        """Standard Pallas-aware loader using make_aware_loader().

        Subclasses with pallas_view_id can use this helper to avoid
        reimplementing the context management logic.
        """
        return PallasViewTracker.make_aware_loader(
            self.data.make_loader(),
            self.make_reindexer(),
            getattr(self, 'pallas_view_id', None)
        )

    def _make_pallas_aware_indexer(
        self,
        expand_callback: Optional[Callable[[Sequence[Expr], Expr], Expr]] = None
    ) -> Callable[[Sequence[Expr]], Expr]:
        """Standard Pallas-aware indexer using make_aware_indexer().

        Subclasses with pallas_view_id can use this helper to avoid
        reimplementing the context management logic.
        """
        return PallasViewTracker.make_aware_indexer(
...
class ExpandView(BaseView):
    size: Sequence[Expr]
    # Optional view_id for Pallas backend to track transforms through ExpandView nodes
    pallas_view_id: Optional[int] = None
    ...
    def create(cls, x: IRNode, new_size: Sequence[_IntLike]) -> BaseView:
        ...
        # Helper to compute which dims are being expanded
        def compute_expand_dims(old_sizes, new_sizes):
            skip = len(new_sizes) - len(old_sizes)
            expand_dims = []
            for i, (old_s, new_s) in enumerate(zip(
                [None] * skip + list(old_sizes),
                new_sizes
            )):
                if old_s is None or V.graph.sizevars.is_size_one_or_false(old_s):
                    if not V.graph.sizevars.is_size_one_or_false(new_s):
        ...
        if is_storage_and_layout(x):
            ...
            # Use centralized Pallas registration
            expand_dims = compute_expand_dims(old_layout.size, new_size)
            pallas_view_id = PallasViewTracker.register(x, {
                "op": "expand",
                "expand_dims": expand_dims,
                "target_shape": [PallasViewTracker.safe_int(s) for s in new_size],
            })

            # Build pallas_origin for ReinterpretView (only if Pallas is active)
            pallas_origin = None
            if pallas_view_id is not None:
                pallas_origin = {
                    "op": "expand",
                    "actual_shape": [int(s) for s in old_layout.size],
                    "target_shape": [PallasViewTracker.safe_int(s) for s in new_size],
                }
            ...
            return ReinterpretView(data=storage, layout=new_layout, pallas_origin=pallas_origin)

        # Non-storage case: use centralized Pallas registration
        actual = x.get_size() if hasattr(x, "get_size") else []
        expand_dims = compute_expand_dims(actual, new_size)
        pallas_view_id = PallasViewTracker.register(x, {
            "op": "expand",
            "expand_dims": expand_dims,
            "target_shape": [PallasViewTracker.safe_int(s) for s in new_size],
        ...
        return ExpandView(data=x, size=new_size, pallas_view_id=pallas_view_id)
    ...
    def make_indexer(self) -> Callable[[Sequence[Expr]], Expr]:
        """Override to add expand dimension info for Pallas backend."""
        inner = self.data.make_indexer()
        reindex = self.make_reindexer()

        if not PallasViewTracker.is_active() or self.pallas_view_id is None:
            return PallasViewTracker.make_aware_indexer(inner, reindex, None)

        # Pallas-specific: prepare expand dimension markers
        target = self.get_size()
        actual = self.data.get_size()
        skip = len(target) - len(actual)
        view_id = Integer(self.pallas_view_id)

        # Determine which dimensions are expand dimensions
        expand_dim_positions = []
        for i in range(skip):
            expand_dim_positions.append(i)
        for i in range(len(actual)):
            if actual[i] == 1:
                expand_dim_positions.append(skip + i)

        def expand_callback(idx: Sequence[Expr], base_expr: Expr) -> Expr:
            for out_dim in expand_dim_positions:
                if out_dim < len(idx):
                    base_expr = base_expr + idx[out_dim] * PallasStride(
        ...
        return PallasViewTracker.make_aware_indexer(inner, reindex, self.pallas_view_id, expand_callback)

    def make_loader(self) -> Callable[[Sequence[Expr]], OpsValue]:
        """Override to set outer view_id context for Pallas backend."""
        return self._make_pallas_aware_loader()
...
class PermuteView(BaseView):
    ...
    def create(cls, x: IRNode, dims: Sequence[int]) -> BaseView:
        ...
        # Use centralized Pallas registration
        permute_transform = {"op": "permute", "dims": [int(d) for d in dims]}
        pallas_view_id = PallasViewTracker.register(x, permute_transform)
        is_pallas = pallas_view_id is not None

        if is_storage_and_layout(x):
            storage, old_layout = as_storage_and_layout(x)

            # Build pallas_origin for ReinterpretView (only if Pallas is active)
            pallas_origin = permute_transform if is_pallas else None

            # pallas_orig_dims tracks which original dimension each current position came from
            # For permute(1, 0): position 0 came from orig dim 1, position 1 came from orig dim 0
            pallas_orig_dims_val = [int(d) for d in dims] if is_pallas else None
            new_layout = FixedLayout(
                old_layout.device,
                old_layout.dtype,
                ...
                old_layout.is_pinned,
                pallas_view_id,
                pallas_orig_dims_val,
            )

            return ReinterpretView(data=storage, layout=new_layout, pallas_origin=pallas_origin)

        # Non-storage case
        return PermuteView(data=x, dims=dims, pallas_view_id=pallas_view_id)
    ...
    def make_indexer(self) -> Callable[[Sequence[Expr]], Expr]:
        """Override to propagate pallas_view_id through the index expression."""
        return self._make_pallas_aware_indexer()
...
class SqueezeView(BaseView):
    ...
    def create(cls, x: IRNode, *, dim: Optional[int] = None) -> IRNode:
        ...
        if is_storage_and_layout(x):
            ...
            new_size = []
            new_stride = []
            squeezed_dims = []  # Track which dims are squeezed for Pallas
            if dim is not None:
            ...
            # Generate view_id for Pallas using consolidated helper
            transforms = [{"op": "squeeze", "dim": sq_dim} for sq_dim in reversed(squeezed_dims)]
            pallas_view_id = PallasViewTracker.register(x, transforms)
        ...
        # Non-storage path: compute view_id using consolidated helper
        # Determine which dims will be squeezed
        old_size = x.get_size()
        squeezed_dims = []
        if dim is None:
            for i, s in enumerate(old_size):
                if V.graph.sizevars.is_size_one_or_false(s):
                    squeezed_dims.append(i)
        else:
            squeezed_dims = [dim]

        # Build transforms and register with consolidated helper
        transforms = [{"op": "squeeze", "dim": sq_dim} for sq_dim in reversed(squeezed_dims)]
        pallas_view_id = PallasViewTracker.register(x, transforms)
        ...
        if dim is None:
            return View.create(
                ...
                pallas_view_id=pallas_view_id,
            )
        else:
            assert x.get_size()[dim] == 1
            return View.create(x, [s for i, s in enumerate(x.get_size()) if i != dim], pallas_view_id=pallas_view_id)
...
class GenericView(BaseView):
    size: Sequence[Expr]
    reindex: Callable[[Sequence[Expr]], Sequence[Expr]]
    # Optional unfold info for Pallas backend: (dim, size, step, input_strides)
    # When set, Pallas generates gather code instead of reshape
    pallas_unfold_info: Optional[tuple[int, int, int, list[int]]] = None
    ...
    def create(
        ...
        return cls(data=x, size=list(new_size), reindex=reindex, pallas_unfold_info=pallas_unfold_info)
    ...
    def make_loader(self) -> Callable[[Sequence[Expr]], OpsValue]:
        """Override to propagate view_id context for Pallas backend.

        For Pallas backend, this method handles two cases:
        1. pallas_unfold_info (for unfold operations)
        2. pallas_view_id (for View/SliceView operations)

        Both cases use PallasViewTracker.context() to propagate the view_id so that
        inner FixedLayout.make_indexer() embeds it in PallasStride markers.
        """
        inner = self.data.make_loader()
        reindex = self.make_reindexer()

        # Check if this instance has pallas_view_id (View and SliceView have this)
        # This enables slice-of-computed-value to propagate the view chain correctly
        pallas_view_id = getattr(self, "pallas_view_id", None)
        if pallas_view_id is not None:
            return PallasViewTracker.make_aware_loader(inner, reindex, pallas_view_id)

        if self.pallas_unfold_info is None:
            # No unfold info - standard behavior
            def loader(idx: Sequence[Expr]) -> OpsValue:
                return inner(reindex(idx))
            return loader

        # Create view_id and store unfold info using entry-based storage
        # This is done at make_loader() time so view_id is stable
        unfold_view_id = PallasViewTracker.next_view_id()
        PallasViewTracker.store_entry(unfold_view_id, {
            "transforms": [{
                "op": "unfold",
                "dim": self.pallas_unfold_info[0],
                "size": self.pallas_unfold_info[1],
                "step": self.pallas_unfold_info[2],
                "input_strides": self.pallas_unfold_info[3],
            }],
            "source_view_id": None,  # Standalone unfold entry
            "output_shape": None,  # Computed at codegen time
        })

        def loader(idx: Sequence[Expr]) -> OpsValue:
            # Set outer_view_id context so FixedLayout.make_indexer() embeds it in PallasStride
            with PallasViewTracker.context(unfold_view_id):
                return inner(reindex(idx))
...
class View(GenericView):
    # Optional view_id for Pallas backend to track transforms through View nodes
    pallas_view_id: Optional[int] = None
    ...
    @classmethod
    def create(cls, x: IRNode, new_size: Sequence[Expr], pallas_view_id: Optional[int] = None) -> IRNode:  # type: ignore[override]
        ...
        if 0 in new_size:
            ...
            return cls(data=x, size=list(new_size), reindex=fake_reindex, pallas_view_id=pallas_view_id)
        # TODO: a new class for FixedTransferLayout that output layout is constrained by input layout
            ...
            # Track reshape in view chain for Pallas backend (only when ndim changes)
            pallas_view_id_for_layout = pallas_view_id
            if PallasViewTracker.is_active() and len(old_size) != len(new_size):
                pallas_view_id_for_layout = PallasViewTracker.register(x, {
                    "op": "reshape",
                    "from_shape": [PallasViewTracker.safe_int(s) for s in old_size],
                    "to_shape": [PallasViewTracker.safe_int(s) for s in new_size],
                })
            ...
            new_layout = FixedLayout(
                ...
                old_layout.offset,
                old_layout.is_pinned,
                pallas_view_id_for_layout,
            )
            return ReinterpretView(data=storage, layout=new_layout)

        # Track reshape in view chain for Pallas backend (non-contiguous path)
        pallas_view_id_for_view = pallas_view_id
        if PallasViewTracker.is_active():
            pallas_view_id_for_view = PallasViewTracker.register(x, {
                "op": "reshape",
                "from_shape": [PallasViewTracker.safe_int(s) for s in old_size],
                "to_shape": [PallasViewTracker.safe_int(s) for s in new_size],
            })
        ...
        return cls(data=x, size=list(new_size), reindex=reindex, pallas_view_id=pallas_view_id_for_view)
...
class ReinterpretView(BaseView):
    ...
    layout: Layout
    # Optional origin info for Pallas codegen - stores where this view came from
    # e.g., {"op": "permute", "dims": [1, 0, 2]} if created from PermuteView
    pallas_origin: Optional[dict] = None
...
class DtypeView(BaseView):
    ...
    def create(cls, x: IRNode, new_dtype: torch.dtype) -> BaseView:
        # DtypeView preserves existing view_id (dtype change doesn't affect indexing)
        pallas_view_id = PallasViewTracker.find_inner_view_id(x) if PallasViewTracker.is_active() else None
        ...
        if is_storage_and_layout(x):
            ...
            return ReinterpretView(data=storage, layout=new_layout)
        return DtypeView(data=x, target_dtype=new_dtype, pallas_view_id=pallas_view_id)
...
class SliceView(View):
    ...
    def create(  # type: ignore[override]
        ...
        # Build slice transform for Pallas
        slice_transform = {
            "op": "slice",
            "dim": dim,
            "start": PallasViewTracker.safe_int(start),
            "end": PallasViewTracker.safe_int(end),
            "step": PallasViewTracker.safe_int(step),
        }
        ...
        if is_storage_and_layout(x):
            ...
            # Use centralized Pallas registration
            pallas_view_id = PallasViewTracker.register(x, slice_transform)
        ...
        # Non-storage path: use centralized Pallas registration
        pallas_view_id = PallasViewTracker.register(x, slice_transform)
        ...
        return SliceView(data=x, size=new_size, reindex=reindex, pallas_view_id=pallas_view_id)
...
class Layout(OutputSpec):
    ...
    def __init__(
        ...
        # Optional view ID for Pallas to link to transform chain
        self.pallas_view_id = pallas_view_id
        # Optional mapping: pallas_orig_dims[i] = original dimension that current position i came from
        # Used by Pallas to track permutations without relying on stride values
...
class FixedLayout(Layout):
    ...
    def make_indexer(self) -> Callable[[Sequence[Expr]], Expr]:
        """A closure containing math to read a given element"""
        return _fixed_indexer(self.size, self.stride, self.offset, self.pallas_view_id, self.pallas_orig_dims)


def _to_fixed_layout(
    layout: "Layout",
    stride: Optional[Sequence[Expr]] = None
) -> FixedLayout:
    """Create FixedLayout from source layout, preserving Pallas tracking.

    Consolidates FixedLayout creation and ensures pallas_view_id is preserved.
    This helper is used by FlexibleLayout.as_*() methods.

    Args:
        layout: Source layout to copy properties from
        stride: Optional new stride (uses layout.stride if not provided)

    Returns:
        A new FixedLayout with pallas_view_id and pallas_orig_dims preserved
    """
    return FixedLayout(
        layout.device,
        layout.dtype,
        layout.size,
        stride if stride is not None else layout.stride,
        layout.offset,
        layout.is_pinned,
        getattr(layout, 'pallas_view_id', None),
        getattr(layout, 'pallas_orig_dims', None),
    )
...
class FlexibleLayout(Layout):
    ...
    def as_stride_order(
        ...
        return _to_fixed_layout(self, new_stride)
...
class ComputedBuffer(OperationBuffer):
    ...
    def simplify_and_reorder(
        ...
        # Check if Pallas backend is active - affects loop reordering and merging
        is_pallas_backend = (
            get_device_type(self) == "cpu"
            and config.cpu_backend in ("pallas", "pallas_v0")
        )
        ...
        def simplify_and_reorder(
            ...
            # Skip loop reordering for Pallas - it needs to preserve OUTPUT shape
            if is_pallas_backend:
                newsizes = list(sizes)
                reindex0 = same_reorder(list(range(len(sizes))))
                reindex1 = same_reorder(list(range(len(sizes))))
            else:
        ...
        # Don't merge loops for Pallas backends - they need N-D structure
        should_merge_loops = (
            not is_gpu(get_device_type(self)) or not config.loop_ordering_after_fusion
        ) and not is_pallas_backend
...
class ExternKernel(InputsKernel):
    ...
    def convert_to_reinterpret_view(cls, x: IRNode) -> ReinterpretView:
        ...
        # Strip PallasStride markers for comparison - they preserve metadata for
        # Pallas codegen but evaluate to their stride value numerically
        def strip_pallas_stride(expr):
            if isinstance(expr, PallasStride):
                return expr.args[0]  # Return the stride value
            elif hasattr(expr, "args") and expr.args:
                return expr.func(*[strip_pallas_stride(arg) for arg in expr.args])
            return expr

        index_stripped = strip_pallas_stride(index)
        if index_stripped != expected:
            log.debug(
        ...
        # Extract pallas_view_id from source view to preserve it
        pallas_view_id = PallasViewTracker.find_inner_view_id(x) if PallasViewTracker.is_active() else None
