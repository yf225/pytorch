    ...
    def call_method(
        ...
        if name == "queue_callback":
            if not torch._dynamo.compiled_autograd.compiled_autograd_enabled:
                unimplemented("queue_callback requires compiled autograd")
            return variables.UserFunctionVariable(
                torch._dynamo.external_utils.FakeCompiledAutogradEngine.queue_callback,
                source=self.source,
            ).call_function(tx, (torch._dynamo.compiled_autograd.compiled_autograd_fake_engine.get_final_callbacks_var(), *args), kwargs)
        else:
