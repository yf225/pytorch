...
Variable::Impl::Impl(at::Tensor data, bool requires_grad, Edge gradient_edge)
    : TensorImpl(
        /* storage */ at::Storage(c10::intrusive_ptr<at::StorageImpl>(data.storage().impl_ptr())),// yf225 TODO: do we really need to increment the refcount? try to remove it and see what happens
        /* storage_offset */ data.getIntrusivePtr()->storage_offset(),
        /* sizes */ data.sizes(),
        /* strides */ data.strides(),
        /* is_contiguous */ data.getIntrusivePtr()->is_contiguous(),
        /* numel */ data.getIntrusivePtr()->numel(),
        /* type_id */ data.type().type_id(), // yf225 TODO: or should this be data.type_id()?
        /* scalar_type */ data.type().scalarType(), // yf225 TODO: or should this be data.scalar_type()?
        /* is_wrapped_number */ data.getIntrusivePtr()->is_wrapped_number(),
        /* is_variable */ true),
...
// yf225 TODO: move these into TensorImpl.cpp
...
void Variable::Impl::set_data(Tensor new_data) {
  ...
  is_variable_ = true;
  storage_ = std::move(new_data.storage());
  storage_offset_ = new_data.getIntrusivePtr()->storage_offset();
  sizes_ = new_data.sizes();
  strides_ = new_data.strides();
  is_contiguous_ = new_data.getIntrusivePtr()->is_contiguous();
  numel_ = new_data.getIntrusivePtr()->numel();
  is_wrapped_number_ = new_data.getIntrusivePtr()->is_wrapped_number();
}
...
void Variable::ViewImpl::rebase_history(Edge gradient_edge) {
  ...
  auto copy_slices = std::make_shared<CopySlices>(
      base_, at::TensorGeometry(at::Tensor(*this)), std::move(gradient_edge.function));
