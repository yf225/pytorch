...
struct TORCH_API Variable : public at::Tensor {
 ...
 private:
  ...
  Variable(c10::intrusive_ptr<at::TensorImpl> tensor_impl);
  Impl* get() const;
};
...
struct TORCH_API Variable::Impl : public at::VariableImplInterface {
  explicit Impl(
...
/// through grad_fn(). All other fields are always valid.
...
inline Variable::Impl* Variable::get() const {
  ...
  return static_cast<Variable::Impl*>(tensor_impl_->get_variable_impl().get());
}
