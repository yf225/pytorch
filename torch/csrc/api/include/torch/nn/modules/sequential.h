...
class NamedModule {
 public:
  /// We don't allow constructing an empty `NamedModule`.
  NamedModule() = delete;

  /// Creates a `NamedModule` from a (boxed) `Module`.
  template <typename ModuleType>
  NamedModule(std::string name, std::shared_ptr<ModuleType> module_ptr) : NamedModule(std::move(name), AnyModule(std::move(module_ptr))) {}

  /// Creates a `NamedModule` from a `Module`, moving or copying it
  /// into a `shared_ptr` internally.
  // For the use of `std::remove_reference<M>::type`:
  // we need to get rid of any reference components for make_unique.
  template <typename M, typename = torch::detail::enable_if_module_t<M>>
  NamedModule(std::string name, M&& module) : NamedModule(std::move(name), std::make_shared<typename std::remove_reference<M>::type>(std::forward<M>(module))) {}

  /// Creates a `NamedModule` from a `Module` that is unwrapped from
  /// a `ModuleHolder`.
  template <typename M>
  NamedModule(std::string name, const ModuleHolder<M>& module_holder) : NamedModule(std::move(name), module_holder.ptr()) {}
 ...
 private:
  /// Creates a `NamedModule` from a type-erased `AnyModule`.
  NamedModule(std::string name, AnyModule any_module) {
    name_ = std::move(name);
...
// NOTE: We might wonder why we need to have the `NamedModule` class and have
// the `modules_ordered_dict()` function signature be `modules_ordered_dict(std::initializer_list<NamedModule> named_modules)`,
...
// what the types of its elements are. Instead, we have to pass the braced-init list as a whole to the `NamedModule` constructors,
...
inline torch::OrderedDict<std::string, AnyModule> modules_ordered_dict(std::initializer_list<NamedModule> named_modules) {
