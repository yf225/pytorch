...
void Module::to_impl(Ts&&... ts) {
  ...
  for (auto& parameter : parameters_) {
    // parameter->set_data(autograd::Variable(*parameter).data().to(ts...));  // yf225 TODO: used set_data()
  }
  // Then move every buffer to the new dtype/device.
  for (auto& buffer : buffers_) {
    // buffer->set_data(autograd::Variable(*buffer).data().to(ts...));  // yf225 TODO: used set_data()
  }
}
