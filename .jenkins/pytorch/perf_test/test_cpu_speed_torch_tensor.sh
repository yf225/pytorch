...
test_cpu_speed_torch_tensor () {
  ...
  git clone https://github.com/yf225/perf-tests.git -b module_tests

  if [ "$1" == "compare_with_baseline" ]; then
    ARGS="--compare ../cpu_runtime.json"
  elif [ "$1" == "compare_and_update" ]; then
    ARGS="--compare ../cpu_runtime.json --update ../new_cpu_runtime.json"
  elif [ "$1" == "update_only" ]; then
    ARGS="--update ../new_cpu_runtime.json"
  fi
